...
function isWithinRange(date: number, lo: number, hi: number): boolean {
  return +date > +lo && +date < +hi
}

function inefficientlyFilterTitlesByYearMonth(
  titles: Title[],
  year: number,
  month: number,
): Title[] {
  return titles.filter(({ launch_date }) => {
    const date = +new Date(launch_date + 'Z') // Tricky trick with UTC timestamp
    const lo = Date.UTC(year, month - 1, 1) - 1
    const hi = Date.UTC(year, month, 1)
    return isWithinRange(date, lo, hi)
  })
}
...
export default class TitlesService {
  getTitles(year: number, month: number) {
    return new Promise(resolve => {
      setTimeout(() => {
        const allTitles = getMockTitles()
        const matchedTitles = inefficientlyFilterTitlesByYearMonth(allTitles.data, year, month)
        resolve({ data: matchedTitles })
      }, Math.random() * 500)
    })
  }
}
